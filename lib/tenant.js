  ...
  */
  ...
  static discover(defaults, rootPath) {
    ...
    siteNames.forEach(function foreachSiteName(siteName) {
      ...
      try {
        let settings = require(resolvedSitePath);
      }
      catch(ex) {
  ...
  static resolve(request) {
    let shellNames = Object.getOwnPropertyNames(Tenant.list);
    ...
    for (let i = 0; i < shellNames.length; i++) {
      let shell = Tenant.list[shellNames[i]];
  ...
  static listen() {
    ...
    for (let shellName in Tenant.list) {
      let tenant = Tenant.list[shellName];
      ...
      for (let i = 0; i < hosts.length; i++) {
        ...
        let serverKey = host + currentPort + (tenant.key || "") + (tenant.cert || "") + (tenant.pfx || "");
  ...
  return function defaultHandler(req, res) {
    ...
    d.on('error', function onError(err) {
      ...
      try {
        ...
        let killTimer = setTimeout(function processKill() {
          process.exit(1);
        }, 30000);
